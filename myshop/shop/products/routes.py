...
def result():
    ...
    products = Addproduct.query.msearch(searchword, fields=['name', 'desc'], limit=12)
...
def deletebrand(id):
    ...
    flash('Brand cannot be deleted', 'warning')
...
def get_category(id):
    ...
    get_cat_pro = Addproduct.query.filter_by(category=get_cat).paginate(page=page, per_page=12)
...
def deletecat(id):
    ...
    flash('Category cannot be deleted', 'warning')
...
def addproduct():
    ...
    if request.method == 'POST':
        ...
        flash(f'{name} Product Added Succefully', 'success')
        db.session.commit()
...
def deleteproduct(id):
    ...
    flash('Product cannot be deleted', 'warning')
